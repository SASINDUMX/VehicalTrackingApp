...
DECLARE
  ...
  v_records JSONB;
BEGIN
  v_kpis := public.get_service_report_kpis(p_start_date, p_end_date, p_branch_id, p_status);

  SELECT COALESCE(jsonb_agg(rec), '[]'::jsonb) INTO v_records
  FROM (
    SELECT 
      v.id,
      v.vehicle_no,
      ...
      v.current_zone,
      v.is_finished,
      (v.is_finished = TRUE OR v.current_zone = 'inspection' OR v.effective_completed_at IS NOT NULL) AS is_effective_done,
      v.is_paused,
      v.pause_reason,
      v.is_booking,
      ...
      v.urgent_note,
      COALESCE((
        SELECT STRING_AGG(t.task_name, ', ')
        FROM public.vehicle_tasks t
        WHERE t.vehicle_id = v.id AND t.is_completed = TRUE
      ), 'None') AS completed_tasks_str
  ...
  RETURN jsonb_build_object(
    'summary', v_kpis,
    'workshopBay', v_kpis->'workshopBay',
    'alignmentBay', v_kpis->'alignmentBay',
    'hoistBay', v_kpis->'hoistBay',
    'records', v_records
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
